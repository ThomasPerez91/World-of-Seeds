...
@pytest.mark.asyncio
async def test_auth_has_no_theme_preference_or_update_route(
    ...
    assert "preferred_theme" not in User.__table__.columns
    await login(client, "theme-user", "correct-horse-battery")
    response = await client.get("/api/v1/auth/me")
    assert response.status_code == 200
    assert "preferred_theme" not in response.json()["user"]
    removed = await client.patch(
        "/api/v1/auth/theme", json={"preferred_theme": "light"}, headers=csrf_header(client)
    )
