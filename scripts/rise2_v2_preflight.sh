...
# NewGreedy must not persist complete tracker flows because announce URLs can
# contain passkeys. Keep the pilot's durable stats policy at the same security
# boundary so preflight and the declarative init gate share one validator.
python3 "$repository/scripts/rise2_v2_newgreedy_policy.py" "$newgreedy_config" \
  || fail "NewGreedy policy requires flow_detail=0, persist_stats=true and auto_purge_stopped=false"
