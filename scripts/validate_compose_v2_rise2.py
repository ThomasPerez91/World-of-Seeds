...
def _mounts_by_target(
    service: Mapping[str, Any], name: str
...
def validate_config(config: Mapping[str, Any]) -> None:
    bootstrap = runpy.run_path(
        str(Path(__file__).with_name("rise2_v2_qb_bootstrap.py"))
    )
    try:
        ...
        raise ComposeRise2PolicyError(
            "qB bootstrap Host/CSRF/proxy policy is unsafe"
        ) from None
    if config.get("secrets"):
        raise ComposeRise2PolicyError(
            "credentials require stable private directory mounts"
        )
    ...
    if (
        _mapping(networks["torrent-egress"], "networks.torrent-egress").get("internal")
        is True
    ):
        raise ComposeRise2PolicyError(
            "torrent-egress must provide external connectivity"
        )
    ...
    if _network_names(ingress) != {"edge", "monitoring-edge"}:
        raise ComposeRise2PolicyError(
            "ingress may bridge only edge and monitoring-edge"
        )
    if ingress.get("cap_drop") != ["ALL"] or ingress.get("cap_add") != [
        "NET_BIND_SERVICE"
    ]:
    ...
    for name, raw in services.items():
        ...
        if name == "ingress":
            if not isinstance(ports, list) or len(ports) != 3:
                raise ComposeRise2PolicyError(
                    "ingress must publish HTTP and HTTPS only"
                )
        elif ports:
    ...
    private_source = str(
        qbittorrent_mounts.get("/bootstrap/private", {}).get("source", "")
    )
    if not private_source.endswith(".runtime/qb"):
        raise ComposeRise2PolicyError(
            "qB requires its preflight-derived private directory"
        )
    ...
    for mounts in (qbittorrent_mounts, init_mounts_qb):
        for target, filename in bootstrap_sources.items():
            ...
            if (
                ...
                raise ComposeRise2PolicyError(
                    "qB bootstrap inputs must be read-only binds"
                )
            ...
            if mount.get("source") != qbittorrent_mounts.get(target, {}).get("source"):
                raise ComposeRise2PolicyError(
                    "qB init and runtime must share bootstrap inputs"
                )
        ...
        if (
            ...
            raise ComposeRise2PolicyError(
                "qB profile must retain its persistent writable volume"
            )
    ...
    if set(qbittorrent_environment) != {
        ...
        raise ComposeRise2PolicyError(
            "qB must not receive inline credentials or policy overrides"
        )
    ...
    if (
        ...
        raise ComposeRise2PolicyError(
            "qBittorrent may mount only the exported public CA volume"
        )
    ...
    for required in (
        ...
        if required not in qbittorrent_command:
            raise ComposeRise2PolicyError(
                "qBittorrent public CA bootstrap is incomplete"
            )
    ...
    if (
        _mapping(
            qbittorrent_depends_on.get("qbittorrent-init"), "qB init dependency"
        ).get("condition")
    ...
    for name, raw in services.items():
        ...
        if name == "cadvisor" and not privileged:
            raise ComposeRise2PolicyError(
                "cAdvisor requires its approved privileged mode"
            )
    ...
    for name in ("api", "worker", "scheduler", "migrate"):
        ...
        if service.get("secrets"):
            raise ComposeRise2PolicyError(
                "replaceable credential file mounts are forbidden"
            )
        credentials_mount = (
        ...
        if name in {"api", "worker", "scheduler"}:
            if (
                ...
                raise ComposeRise2PolicyError(
                    f"{name} requires the private registry directory"
                )
            if name == "api":
                if environment.get("WOS_INTEGRATION_ACCOUNTS_FILE") != (
                    "/run/secrets/integration_registry"
                ):
                    raise ComposeRise2PolicyError(
                        "api must read the mounted integration registry file"
                    )
            else:
                module = "app.worker" if name == "worker" else "app.scheduler_service"
                if service.get("command") != [
                    "python",
                    "/bootstrap/integration-entrypoint.py",
                    module,
                ]:
                    raise ComposeRise2PolicyError(
                        "integration process must load its secret only at runtime"
                    )
                entry = _mounts_by_target(service, name).get(
                    "/bootstrap/integration-entrypoint.py", {}
                )
                if (
                    entry.get("read_only") is not True
                    or Path(str(entry.get("source", ""))).name
                    != "rise2_v2_integration_entrypoint.py"
                ):
                    raise ComposeRise2PolicyError(
                        "integration secret loader mount is missing"
                    )
        elif credentials_mount:
            raise ComposeRise2PolicyError(
                f"{name} must not receive integration credentials"
            )
    ...
    if _mapping(services["api"], "api").get("command") is not None:
        raise ComposeRise2PolicyError(
            "api must retain the measured single-process entry point"
        )
    ...
    api_networks = _mapping(
        _mapping(services["api"], "api").get("networks"), "api.networks"
    )
    ...
    if (
        _mapping(api_networks.get("edge"), "api.networks.edge").get("ipv4_address")
        != "172.30.0.3"
    ):
    ...
    if (
        len(configs) != 1
        or _mapping(configs[0], "edge subnet").get("subnet") != "172.30.0.0/24"
    ):
        raise ComposeRise2PolicyError(
            "edge must retain its dedicated trusted-proxy subnet"
        )
    ...
    if newgreedy.get("user") not in (None, ""):
        raise ComposeRise2PolicyError(
            "NewGreedy must retain the validated image root user"
        )
    ...
    if len(groups) != 1 or re.fullmatch(r"[1-9][0-9]*", str(groups[0])) is None:
        raise ComposeRise2PolicyError(
            "NewGreedy may add only its numeric config-reader group"
        )
    ...
    if config_bind.get("create_host_path") is True:
        raise ComposeRise2PolicyError(
            "NewGreedy config bind must fail when its source is absent"
        )
    state_targets = {
    ...
    for target, filename in state_targets.items():
        ...
        if mount.get("type") != "bind" or mount.get("read_only") is True:
            raise ComposeRise2PolicyError(
                f"NewGreedy state must be a writable bind: {target}"
            )
        bind = _mapping(mount.get("bind"), f"NewGreedy state bind: {target}")
        if bind.get("create_host_path") is True:
            raise ComposeRise2PolicyError(
                f"NewGreedy state bind must fail if absent: {target}"
            )
        if not source.endswith(f"/{filename}"):
            raise ComposeRise2PolicyError(
                f"NewGreedy state source is invalid: {target}"
            )
        state_parents.add(source[: -len(filename) - 1])
    if len(state_parents) != 1:
        raise ComposeRise2PolicyError(
            "NewGreedy state files must share one persistent directory"
        )
    ...
    if (
        ...
        raise ComposeRise2PolicyError(
            "NewGreedy CA must use its writable persistent volume"
        )
    ...
    for item in _sequence(newgreedy.get("tmpfs", []), "newgreedy.tmpfs"):
        if str(item).split(":", 1)[0] in forbidden_tmpfs:
            raise ComposeRise2PolicyError(
                "NewGreedy persistent paths must not use tmpfs"
            )
    ...
    if str(state_mount.get("source", "")) not in state_parents:
        raise ComposeRise2PolicyError(
            "NewGreedy init and runtime must share the state directory"
        )
    ...
    if ca_export.get("image") != newgreedy.get("image"):
        raise ComposeRise2PolicyError(
            "NewGreedy CA export must use the runtime image digest"
        )
    if (
    ...
    if (
        ...
        raise ComposeRise2PolicyError(
            "CA export source must be the read-only private CA volume"
        )
    ...
    if (
        ...
        raise ComposeRise2PolicyError(
            "CA export target must be the writable public CA volume"
        )
    ...
    if "mitmproxy-ca.pem" in export_command:
        raise ComposeRise2PolicyError(
            "CA export must never copy the private-key-bearing CA file"
        )
    export_depends_on = _mapping(
        ca_export.get("depends_on"), "newgreedy-ca-export.depends_on"
    )
    ...
    for forbidden in (
        ...
        if forbidden in payload:
            raise ComposeRise2PolicyError(
                f"Rise2 reuses forbidden V1 or unsafe state: {forbidden}"
            )
