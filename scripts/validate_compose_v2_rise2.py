...
def validate_config(config: Mapping[str, Any]) -> None:
    ...
    if qbittorrent.get("security_opt") != ["no-new-privileges:true"]:
        raise ComposeRise2PolicyError("qBittorrent must retain no-new-privileges")
    if any(
        re.fullmatch(r"[1-9][0-9]*", str(qbittorrent_environment.get(key, ""))) is None
        for key in ("PUID", "PGID")
    ):
        raise ComposeRise2PolicyError("qBittorrent runtime UID/GID must be non-root")
