...
describe("FileBrowser", () => {
  ...
  it.each([320, 375, 390, 430])(
    ...
    async (width) => {
      ...
      const longName = `${"Film.Name.2026.MULTi.TRUEFRENCH.2160p.UHD.BluRay.REMUX.DV.HDR.HEVC.DTS-HD.MA.7.1-GROUP.".repeat(3)}mkv`;
  ...
  it("bloque le double clic pendant une mise en corbeille lente", async () => {
    let releaseTrash!: () => void;
    const pendingTrash = new Promise<void>((resolve) => { releaseTrash = resolve; });
    let trashCalls = 0;
    vi.stubGlobal(
      "fetch",
      vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
